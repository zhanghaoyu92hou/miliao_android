...
public class ScannerActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private final int MESSAGE_DECODE_FROM_BITMAP_MM = 1;
    private final int MESSAGE_DECODE_FROM_BITMAP_MMM = 2;
    ...
    // 第一次失败
    public void handDecode(final int result) {
        if (result == 1 && !TextUtils.isEmpty(imagePath)) {
            try {
                Bitmap bitmap = DecodeUtils.compressPicture1(this, Uri.fromFile(new File(imagePath)));
                Message message = mHandler.obtainMessage(MESSAGE_DECODE_FROM_BITMAP_MM, bitmap);
                mHandler.sendMessage(message);
            } catch (Exception e) {
                Toast.makeText(this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        } else if (result == 2 && !TextUtils.isEmpty(imagePath)) {
            try {
                Bitmap bitmap = DecodeUtils.compressPicture2(this, Uri.fromFile(new File(imagePath)));
                Message message = mHandler.obtainMessage(MESSAGE_DECODE_FROM_BITMAP_MMM, bitmap);
                mHandler.sendMessage(message);
            } catch (Exception e) {
                Toast.makeText(this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case REQUEST_CODE_GET_PIC_URI:
                    ...
                    imagePath = UriUtils.getPicturePathFromUri(ScannerActivity.this, uri);
                    // 对获取到的二维码照片进行压缩
    ...
    private static class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (activity != null) {
                ...
                } else if (msg.what == activity.MESSAGE_DECODE_FROM_BITMAP_MM) {
                    Bitmap bm = (Bitmap) msg.obj;
                    DecodeUtils.DecodeAsyncTask1 decodeAsyncTask = new DecodeUtils.DecodeAsyncTask1(activity);
                    decodeAsyncTask.execute(bm);
                } else if (msg.what == activity.MESSAGE_DECODE_FROM_BITMAP_MMM) {
                    Bitmap bm = (Bitmap) msg.obj;
                    DecodeUtils.DecodeAsyncTask2 decodeAsyncTask = new DecodeUtils.DecodeAsyncTask2(activity);
                    decodeAsyncTask.execute(bm);
                }
            }
        }
