package com.iimm.miliao.ui.map;
...
import com.iimm.miliao.map.GoogleMapHelper;
...
public class MapPickerActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        if (BuildConfig.DEBUG) {
            com.iimm.miliao.util.LogUtils.log("onCreate");
        }
        ...
        initView();
        if (BuildConfig.DEBUG) {
            com.iimm.miliao.util.LogUtils.log("after create");
        }
    }
    ...
    private void initMap() {
        ...
        if (mapHelper == null) {
            ToastUtil.showToast(MapPickerActivity.this, getString(R.string.failed_to_get_location_information));
            return;
        }
    ...
    private void loadMapDatas(MapHelper.LatLng latLng) {
        ...
        mapHelper.requestPlaceList(latLng, new MapHelper.OnSuccessListener<List<MapHelper.Place>>() {
            ...
            public void onSuccess(List<MapHelper.Place> places) {
                runOnUiThread(() -> {
                    if (mapHelper instanceof GoogleMapHelper) {
                        if (places.size() > 1) {
                            MapHelper.Place place = places.get(1);
