 ...
 * web
 */
public class WebView2Activity extends BaseActivity implements OnPermissionClickListener {

    public static final String EXTRA_URL = "url";
    public static final String EXTRA_DOWNLOAD_URL = "download_url";
    public static final String EXTRA_URL_POST = "post";
    ...
    public static void start(Context ctx, String url) {
        Intent intent = new Intent(ctx, WebView2Activity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            ...
            mTypePost = getIntent().getStringExtra(EXTRA_URL_POST);
    ...
    protected void onStop() {
        ...
        if (!Constants.SUPPORT_FLOATING_WINDOW){
            if (IS_FLOATING) {
                getCurrentUrl();
                startService(new Intent(WebView2Activity.this, WindowShowService.class));
            }
        }
    }
    ...
    private void init() {
        ...
        initEvent();
        if (!Constants.SUPPORT_FLOATING_WINDOW){
            stopService(new Intent(WebView2Activity.this, WindowShowService.class));
        }
    }
    ...
    private void initView() {
        ...
        if (TextUtils.isEmpty(mTypePost)){
            mWebView.loadUrl(mUrl);
        }else {
            mWebView.postUrl(mUrl, EncodingUtils.getBytes(mTypePost, "UTF-8"));
        }
        ...
        mWebView.setDownloadListener(new DownloadListener() {
            ...
            public void onDownloadStart(String url, String userAgent, String contentDisposition, String mimetype, long contentLength) {
                try {
                    ...
                    // 无论如何不要崩溃，比如没有浏览器，
                    ToastUtil.showToast(WebView2Activity.this, R.string.download_error);
                }
            }
        });
...
//        mWebView.addJavascriptInterface(jsSdkInterface, "AndroidWebView");
    ...
    private void initEvent() {
        mTitleRightIv.setOnClickListener(view -> {
            WebMoreDialog mWebMoreDialog = new WebMoreDialog(WebView2Activity.this, getCurrentUrl(), new WebMoreDialog.BrowserActionClickListener() {
                @Override
                ...
                public void copyLink() {
                    ...
                    ToastUtil.showToast(WebView2Activity.this, getString(R.string.tip_copied_to_clipboard));
                }
                ...
                public void shareWechat() {
                    ...
                    ShareSdkHelper.shareWechat(
                            WebView2Activity.this, title, url, url, coreManager.getConfig().wechatAppId
                    );
                }
                ...
                @Override
                public void shareQQ() {
                    AndPermissionUtils.AuthorizationStorage(WebView2Activity.this, WebView2Activity.this);
                }
            });
            mWebMoreDialog.show();
        });
...
//            jsSdkInterface.release();
    ...
    private void showFloating() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M
            ...
            PermissionDialog.show(WebView2Activity.this, "悬浮窗权限", "您的手机没有授予悬浮窗权限，请开启后再试", "暂不开启", "现在去开启");
        }
    }
    ...
    private void initChatByContent(String content, int type) {
        ...
        if (ChatMessageDao.getInstance().saveNewSingleChatMessage(mLoginUserId, mNewUserId, message)) {
            Intent intent = new Intent(WebView2Activity.this, InstantMessageActivity.class);
            intent.putExtra("fromUserId", mNewUserId);
            intent.putExtra("messageId", message.getPacketId());
            startActivity(intent);
            finish();
        } else {
            ToastUtil.showToast(mContext, "消息封装失败");
        }
    }
        ...
        HtmlFactory.instance().queryImage(str, new HtmlFactory.DataListener<String>() {// 检索该网页包含的图片
            ...
            public void onResponse(List<String> data, String title) {
                if (data != null && data.size() > 0) {
                    // 弹窗让用户选择图片 作为链接封面
                    SelectImageDialog dialog = new SelectImageDialog(WebView2Activity.this, getCurrentUrl(), data, url -> initChatByUrl(url));
                    dialog.show();
                } else {
                    initChatByUrl("");
                }
    ...
    private void shareMoment() {
        Intent intent = new Intent(WebView2Activity.this, SendShuoshuoActivity.class);
    ...
    private void report() {
        ComplaintDialog complaintDialog = new ComplaintDialog(this, report -> {
            ...
            DialogHelper.showDefaulteMessageProgressDialog(WebView2Activity.this);
            ...
            HttpUtils.get().url(coreManager.getConfig().USER_REPORT)
                    ...
                    .execute(new BaseCallback<Void>(Void.class) {
                        ...
                        public void onResponse(ObjectResult<Void> result) {
                            ...
                            if (result.getResultCode() == 1) {
                                ToastUtil.showToast(WebView2Activity.this, R.string.report_success);
                            }
                        }
    ...
    private class MyJsSdkListener implements JsSdkInterface.Listener {
        ...
        public void onUpdateShareData(String shareBeanContent) {
            WebView2Activity.this.shareBeanContent = shareBeanContent;
        }
    ...
    public IUiListener qqShareListener = new IUiListener() {
        ...
        public void onComplete(Object response) {
            ...
            ToastUtil.showToast(WebView2Activity.this, response.toString());
        }

        @Override
        public void onError(UiError e) {
            // TODO Auto-generated method stub
            ToastUtil.showToast(WebView2Activity.this, e.errorMessage);
        }
    };
    ...
    public void onSuccess() {
        ...
        mTencent = Tencent.createInstance(BuildConfig.QQ_APP_ID, WebView2Activity.this);
        String title = mTitleTv.getText().toString().trim();
        String url = getCurrentUrl();
        ShareSdkHelper.shareQQ(mTencent, WebView2Activity.this, title,
                url,
