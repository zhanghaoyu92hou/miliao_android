...
import com.iimm.miliao.bean.assistant.KeyWord;
...
public class GroupAssistantDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (groupAssistant == null) {
            ToastUtil.showToast(mContext, getString(R.string.tip_get_detail_error));
            finish();
            return;
        }
        ...
        HttpUtils.get().url(CoreManager.requireConfig(mContext).ROOM_DELETE_GROUP_HELPER)
                ...
                .execute(new BaseCallback<Void>(Void.class) {
                    ...
                    public void onResponse(ObjectResult<Void> result) {
                        DialogHelper.dismissProgressDialog();
                        if (result != null && result.getResultCode() == 1) {
                            ToastUtil.showToast(mContext, getString(R.string.delete) + getString(R.string.success));
                            EventBus.getDefault().post(new EventNotifyByTag("GroupAssistant"));
                            changeVisible(false);
                        } else {
                            if (result != null && !TextUtils.isEmpty(result.getResultMsg())) {
