...
public class SearchImageVideoContent extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mSearchType == TYPE_IMAGE) {
            // 全局变量，用于滑动展示
            imageList = ChatMessageDao.getInstance().queryChatMessageByType(coreManager.getSelf().getUserId(), mSearchObject, Constants.TYPE_IMAGE);
            // 根据timeSend排序
            ...
            List<ChatMessage> videoList = ChatMessageDao.getInstance().queryChatMessageByType(coreManager.getSelf().getUserId(), mSearchObject, Constants.TYPE_VIDEO);
            // 根据timeSend排序
            Comparator<ChatMessage> comparator = (o1, o2) -> (int) (o2.getDoubleTimeSend() - o1.getDoubleTimeSend());
