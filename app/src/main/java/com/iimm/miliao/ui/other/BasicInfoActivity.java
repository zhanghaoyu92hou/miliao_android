...
import com.iimm.miliao.ui.message.single.PersonDataSetActivity;
import com.iimm.miliao.ui.message.single.SetRemarkActivity;
...
import com.iimm.miliao.view.BasicInfoWindow;
...
public class BasicInfoActivity extends BaseActivity implements NewFriendListener {
    ...
    private static final int REQUEST_CODE_PERSONDATASET = 1000;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mUserId.equals(Constants.ID_SYSTEM_MESSAGE)) {// 客服公众号
            findViewById(R.id.part_1).setVisibility(View.VISIBLE);
            RoundedImageView roundedImageView = findViewById(R.id.the_public_riv);
            AvatarHelper.getInstance().displayAvatar(mUserId, roundedImageView);
    ...
    private void initEvent() {
        ivRight.setOnClickListener(new NoDoubleClickListener() {
            ...
            public void onNoDoubleClick(View view){
                ...
                Intent intent = new Intent(BasicInfoActivity.this, PersonDataSetActivity.class);
                intent.putExtra("user", mUser);
                intent.putExtra("userId", mUserId);
                startActivityForResult(intent,REQUEST_CODE_PERSONDATASET);
...
//                        0);
        ...
        mRemarkLayout.setOnClickListener(new NoDoubleClickListener() {
            @Override
            public void onNoDoubleClick(View view){
        ...
        rl_describe.setOnClickListener(new NoDoubleClickListener() {
            @Override
            public void onNoDoubleClick(View view){
                BasicInfoActivity.this.start();
            }
        ...
        findViewById(R.id.look_bussic_cicle_rl).setOnClickListener(new NoDoubleClickListener() {
            @Override
            public void onNoDoubleClick(View view){
                if (mUser != null) {
        ...
        erweima.setOnClickListener(v -> {
            if (mUser != null) {
                ...
                DialogHelper.showQRDialog(BasicInfoActivity.this, mUser.getNickName(), "-1", coreManager.getConfig().website, mUser.getUserId(), null);
...
//                startActivity(intent);
    ...
    private void updateUI() {
        ...
        if (coreManager.getConfig().IS_OPEN_TEL_NUM == 0) {
        ...
        birthday_tv.setText(TimeUtils.time_s_long_2_str(mUser.getBirthday()));
        if (mUser.getShowLastLoginTime() > 0) {
            online_rl.setVisibility(View.VISIBLE);
            online_tv.setText(TimeUtils.getFriendlyTimeDesc(this, mUser.getShowLastLoginTime()));
        } else {
        ...
        if (isMyInfo) {
            ...
            if (mUser.getFriends() == null) {// 陌生人
                ...
                int isFriendAdd = PreferenceUtils.getInt(MyApplication.getInstance(), MyApplication.getLoginUserId() + "_" + Constants.IS_FRIEND_ADD, 0);
                if (mUser.getUserType() != Constants.USER_ROLE_GONG_ZHONG_HAO && mUser.getUserType() != Constants.USER_ROLE_KE_FU && isFriendAdd == 0) {
                    mNextStepBtn.setVisibility(View.GONE);
                } else {
            ...
            Glide.with(MyApplication.getContext())
                    ...
                    .into(new SimpleTarget<GlideDrawable>() {
                        ...
                        public void onLoadFailed(Exception e, Drawable errorDrawable) {
                            DialogHelper.dismissProgressDialog();
                            Log.e(TAG, "加载原图失败：" + mOriginalUrl);// 该用户未设置头像，网页访问该URL也是404
    ...
    public boolean onNewFriend(NewFriendMessage message) {
        ...
        if (message.getType() == Constants.TYPE_PASS) {// 对方同意好友请求 更新当前界面
            loadOthersInfoFromNet();
        }
        return false;
    }
    ...
    public void msgSendSuccess(NewFriendMessage message, String packet) {
        if (addhaoyouid != null && addhaoyouid.equals(packet)) {
            if (isyanzheng == 0) {// 需要验证
                ...
                sayChatMessage.setDoubleTimeSend(TimeUtils.time_current_time_double());
                // 各种Dao都是本地数据库的操作，
                // 这个是朋友表里更新最后一条消息，
                FriendDao.getInstance().updateLastChatMessage(mLoginUserId, Constants.ID_NEW_FRIEND_MESSAGE, sayChatMessage);

                // 这个是新的朋友界面用的，等验证什么的都在新的朋友页面，
                NewFriendDao.getInstance().changeNewFriendState(mUser.getUserId(), Constants.STATUS_10);// 朋友状态
                ...
                addChatMessage.setDoubleTimeSend(TimeUtils.time_current_time_double());
                FriendDao.getInstance().updateLastChatMessage(mLoginUserId, Constants.ID_NEW_FRIEND_MESSAGE, addChatMessage);

                NewFriendDao.getInstance().changeNewFriendState(mUser.getUserId(), Constants.STATUS_22);//添加了xxx
                FriendDao.getInstance().updateFriendContent(mLoginUserId, mUser.getUserId(), InternationalizationHelper.getString("JXMessageObject_BeFriendAndChat"), Constants.TYPE_TEXT, TimeUtils.time_current_time());
                ListenerManager.getInstance().notifyNewFriend(mLoginUserId, message, true);
                        ...
                        if (result.getResultCode() == 1 && result.getData() != null) {
                            if (result.getData().getType() == 1 || result.getData().getType() == 3) {
                                ...
                                NewFriendMessage message = NewFriendMessage.createWillSendMessage(
                                        coreManager.getSelf(), Constants.TYPE_FRIEND, null, mUser);
                                NewFriendDao.getInstance().createOrUpdateNewFriend(message);
                                // 不需要验证的话直接加上，就发个xmpp消息，
                                // 这里最终调用smack的方法发送xmpp消息，
                                ImHelper.sendNewFriendMessage(mUser.getUserId(), message);
    ...
    private void doSayHello(String text) {
        ...
        NewFriendMessage message = NewFriendMessage.createWillSendMessage(coreManager.getSelf(),
                Constants.TYPE_SAYHELLO, text, mUser);
        NewFriendDao.getInstance().createOrUpdateNewFriend(message);
        // 这里最终调用smack的发送消息，
        ImHelper.sendNewFriendMessage(mUser.getUserId(), message);
        addhaoyouid = message.getPacketId();
...
*/
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == REQUEST_CODE_PERSONDATASET) {
            loadOthersInfoFromNet();
        }
    }
