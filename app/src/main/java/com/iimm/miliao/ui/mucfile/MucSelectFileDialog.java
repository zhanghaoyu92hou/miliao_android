...
import com.iimm.miliao.view.DataLoadView;
...
import static com.iimm.miliao.R.id.tv_group;
...
public class MucSelectFileDialog extends Dialog {
    ...
    private void changeDataSource(Map<String, ArrayList> data) {
        ...
        // 还是空数据就弹Toast,
        if (cuttDatas.size() == 0) {
            ToastUtil.showToast(mContext, InternationalizationHelper.getString("JX_NoData"));
        }
    }
                    ...
                    public void onClick(View v) {
                        if (!list.get(childPosition).aCase) {
                            if (opt.size() > maxOpt - 1) {
                                ToastUtil.showToast(mContext, mContext.getString(R.string.select_file_count_place_holder, maxOpt));
                                return;
                            }
                        }
