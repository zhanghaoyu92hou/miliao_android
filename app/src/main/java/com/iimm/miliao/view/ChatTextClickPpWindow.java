...
public class ChatTextClickPpWindow extends PopupWindow {
    ...
    private void hideButton(ChatMessage message, boolean course) {
        ...
        // 图片类型可存表情
        if (type == Constants.TYPE_IMAGE || type == Constants.TYPE_GIF) {
        ...
        // 文本、图片、语音、视频、文件类型可收藏
        if (type == Constants.TYPE_TEXT || type == Constants.TYPE_IMAGE || type == Constants.TYPE_VOICE || type == Constants.TYPE_VIDEO || type == Constants.TYPE_FILE) {
        ...
        // 撤回
        if (isGroup) {
            if ((message.isMySend() || mRole == 1 || mRole == 2) && type != Constants.TYPE_RED && type != Constants.TYPE_READ_EXCLUSIVE && type != Constants.TYPE_CLOUD_RED) {
            ...
            if (!message.isMySend()
                    || type == Constants.TYPE_CLOUD_RED
                    || type == Constants.TYPE_RED
                    || type == Constants.TYPE_READ_EXCLUSIVE
                    || type == Constants.TYPE_TRANSFER
                    || type == Constants.TYPE_CLOUD_TRANSFER
                    || ((type >= Constants.TYPE_IS_CONNECT_VOICE && type <= Constants.TYPE_EXIT_VOICE))) {
    ...
    private boolean judgeTime(long timeSend) {
        return timeSend + 300 < TimeUtils.time_current_time();
    }
}
