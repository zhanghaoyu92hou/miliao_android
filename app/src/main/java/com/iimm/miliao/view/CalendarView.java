...
import com.iimm.miliao.bean.AttrsBean;
import com.iimm.miliao.bean.DateBean;
import com.iimm.miliao.util.CalendarUtil;
import com.iimm.miliao.util.SolarUtil;
import com.iimm.miliao.view.listener.CalendarPagerAdapter;
import com.iimm.miliao.view.listener.CalendarViewAdapter;
import com.iimm.miliao.view.listener.MonthView;
import com.iimm.miliao.view.listener.OnMultiChooseListener;
import com.iimm.miliao.view.listener.OnPagerChangeListener;
import com.iimm.miliao.view.listener.OnSingleChooseListener;
...
public class CalendarView extends ViewPager {
    ...
    private int[] mCurrentDate = new int[2];//记录当前日期的ViewPager position以及当前日期
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < ta.getIndexCount(); i++) {
            ...
            } else if (attr == R.styleable.CalendarView_today_color) {
                mAttrsBean.setColorToday(ta.getColor(attr, mAttrsBean.getColorToday()));
    ...
    public void init() {
        ...
        if (mAttrsBean.isShowCurrentDate()) {
            int[] tempCurrentDate = CalendarUtil.getCurrentDate();
            if (isIllegal(tempCurrentDate)) {
                mCurrentDate[0] = CalendarUtil.dateToPosition(tempCurrentDate[0], tempCurrentDate[1], startDate[0], startDate[1]);
                mCurrentDate[1] = tempCurrentDate[2];
            }
        }
        //多选
     ...
     *
     */
    public void refreshSignInMonthDate(int position, List<String> stringList) {
        if (chooseDate == null) {
            chooseDate = new SparseArray<>();
        }
        if (positions == null) {
        ...
        if (stringList != null) {
            for (String dateStr : stringList) {
                int[] date = CalendarUtil.strToArray(dateStr);
                if (isIllegal(date)) {
                    int datePosition = CalendarUtil.dateToPosition(date[0], date[1], startDate[0], startDate[1]);
                    positions.add(datePosition);
                    setChooseDate(date[2], true, datePosition);
    ...
    private void refreshMonthView(int position) {
        ...
        if (mAttrsBean.getChooseType() == 1) {//多选
            if (chooseDate.get(position) != null) {
                ...
                //如果当前页position  == 今天的日期 position
                if (position == mCurrentDate[0]) {
                    monthView.multiChooseRefreshCurrentDate(mCurrentDate[1]);
                }
            }
        } else {
     ...
     * @return
     */
    public CalendarView showCurrentDate(boolean value) {
        mAttrsBean.setShowCurrentDate(value);
        return this;
    }
