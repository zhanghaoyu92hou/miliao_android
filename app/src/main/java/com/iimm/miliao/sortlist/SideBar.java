...
public class SideBar extends View {
    ...
    private float textSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 12, getContext().getResources().getDisplayMetrics());
    private float yoffset = textSize / 3;
    private int touchColor = SkinUtils.getSkin(getContext()).getAccentColor();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < b.length; i++) {
            paint.setColor(Color.parseColor("#AEAFB3"));
            ...
            if (i == choose) {
                float xPos = width / 2;
                float yPos = singleHeight - yoffset + (i * singleHeight);
                paint.setColor(SkinUtils.getSkin(getContext()).getPrimaryColor());
                canvas.drawCircle(xPos, yPos, xPos, paint);
