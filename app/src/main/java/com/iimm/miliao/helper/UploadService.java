...
public class UploadService {
     ...
     * @return
     */
    public String uploadFile(Context context, CoreManager coreManager, Map<String, String> params, List<String> filePathList) {
        if (coreManager.getConfig().IS_OPEN_OBS_STATUS != 0) {
            String result = OBSUtils.uploadFileList(filePathList, result1 -> {
            }, msg -> {

            }, context, coreManager);
            if (TextUtils.isEmpty(result)) {
                result = uploadFile(coreManager.getConfig().UPLOAD_URL, params, filePathList);
            }
            LogUtil.i("上传OBS成功");
            return result;
        } else {
            LogUtil.i("未配置上传OBS 或上传出错 进入服务器上传");
            String result = uploadFile(coreManager.getConfig().UPLOAD_URL, params, filePathList);
            return result;
        }
    }
