...
public class FriendHelper {
    ...
    public static boolean updateFriendRelationship(String loginUserId, User user) {// 更新两者的关系，因为本地数据可能不正确
        ...
        if (attentionUser == null) {// 服务器上不存在关系
            ...
            if (friend != null) {// 服务器上不存在关系，本地却有
                if (friend.getStatus() != Constants.STATUS_23) {
                    // 从好友表中改变状态
                    // 只改变状态，和被删除一致，避免在其他页面被查询出来，
                    FriendDao.getInstance().updateFriendStatus(loginUserId, friendId, Constants.STATUS_23);
                }
            } else {
            ...
            if (friend == null) {// 本地不存在关系，那么就要插入一条好友记录
                ...
                int status = (attentionUser.getBlacklist() == 0) ? attentionUser.getStatus() : Constants.STATUS_BLACKLIST;
                friend.setStatus(status);
                friend.setVersion(TableVersionSp.getInstance(MyApplication.getInstance()).getFriendTableVersion(loginUserId));// 更新版本
                FriendDao.getInstance().createOrUpdateFriend(friend);

                if (status == Constants.STATUS_ATTENTION) {// 如果是关注（理论上不可能）
                    addAttentionExtraOperation(loginUserId, friendId);
                } else if (status == Constants.STATUS_FRIEND) {   // 如果是好友
                ...
                if (status == friend.getStatus()) {
                    ...
                    if (status == Constants.STATUS_BLACKLIST) {// 如果之前在黑名单中，现在是STATUS_ATTENTION或者STATUS_FRIEND
                        if (friend.getStatus() == Constants.STATUS_ATTENTION) {
                            addAttentionExtraOperation(loginUserId, friendId);
                        } else if (friend.getStatus() == Constants.STATUS_FRIEND) {
                            addFriendExtraOperation(loginUserId, friendId);
                        }
                    } else if (status == Constants.STATUS_ATTENTION) {// 如果之前是关注，现在是黑名单或者好友
                        if (friend.getStatus() == Constants.STATUS_BLACKLIST) {
                            addBlacklistExtraOperation(loginUserId, friendId);
                        } else if (friend.getStatus() == Constants.STATUS_FRIEND) {
                            addFriendExtraOperation(loginUserId, friendId);
                        }
                    } else if (status == Constants.STATUS_FRIEND) {
                        if (friend.getStatus() == Constants.STATUS_BLACKLIST) {
                            addBlacklistExtraOperation(loginUserId, friendId);
                        } else if (friend.getStatus() == Constants.STATUS_ATTENTION) {// 本来是好友，现在变成关注
                            // 消息表中删除
    ...
    public static void downloadCircleMessage(final String loginUserId, final String firendId) {
        ...
        HttpUtils.get().url(CoreManager.requireConfig(MyApplication.getInstance()).USER_CIRCLE_MESSAGE).params(params)
                .build().execute(new ListCallback<CircleMessage>(CircleMessage.class) {
            @Override
            public void onResponse(com.xuan.xuanhttplibrary.okhttp.result.ArrayResult<CircleMessage> result) {
...
//        MyApplication.getInstance().getFastVolley().addDefaultRequest(null, request);
    ...
    public static void removeAttentionOrFriend(String ownerId, String friendId) {
        // 从好友表中改变状态
        FriendDao.getInstance().updateFriendStatus(ownerId, friendId, Constants.STATUS_23);//对方我把加入了黑名单 不删除好友
    ...
    /*public static void beDeleteSeeNewFriend(String loginUserId, String friendId) {
        ...
        if (friend != null) {
            if (friend.getStatus() == Constants.STATUS_FRIEND) {
                friend.setStatus(Constants.STATUS_ATTENTION);
                friend.setContent("");
                // 由好友变为关注，更新一些数据
                FriendDao.getInstance().createOrUpdateFriend(friend);
                // 消息表中删除
