...
public class UserClickableSpanCollection extends ClickableSpan {
    int color = -1;
    private Context context;
    private String userId;
    private String nickName;// 可能也是备注名字，但是不影响，传递到BusinessCircleActivity页面，一样的会查询出备注名

    public UserClickableSpanCollection(Context context, String userId, String nickName) {
     ...
     * @param
     */
    public UserClickableSpanCollection(int color, Context context, String userId, String nickName) {
        if (color != -1) {
            this.color = color;
        }
    ...
    public void updateDrawState(TextPaint ds) {
        if (color == -1) {
            ds.setColor(context.getResources().getColor(R.color.color_96));
        } else {
            ds.setColor(color);
        }
    ...
    public static void setClickableSpan(Context context, SpannableStringBuilder builder, String appendData, String userId) {
        ...
        builder.setSpan(new UserClickableSpanCollection(context, userId, appendData), end - length, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
}