...
public class AnimationUtil {
    ...
    public static void setVisibleForBottom(View view) {
     ...
     * @param view     变更的view
     * @param size     大小   像素
     * @param isHeight 是否时高度变化  ，否则时 宽度变化
     */
    public static void setVisibleChangeSize(View view, float size, boolean isHeight) {
        ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
        if (layoutParams == null) {
            Log.w("AnimationUtil", "无法获取父view信息，动画无效");
            return;
        }
        ValueAnimator valueAnimator = ObjectAnimator.ofFloat(0, size);
    ...
    public static void setGoneChangeSize(View view, float size, boolean isHeight) {
        ViewGroup.LayoutParams layoutParams = view.getLayoutParams();
        if (layoutParams == null) {
            Log.w("AnimationUtil", "无法获取父view信息，动画无效");
            return;
        }
        ValueAnimator valueAnimator = ObjectAnimator.ofFloat(size, 0);
        valueAnimator.setDuration(300);
        valueAnimator.addUpdateListener(animation -> {
            float value = (float) animation.getAnimatedValue();
            if (isHeight) {
    ...
    public static void setGoneForBottom(View view) {
        TranslateAnimation hideAnim = new TranslateAnimation(Animation.RELATIVE_TO_SELF, 0.0f,
                Animation.RELATIVE_TO_SELF, 0.0f,
                Animation.RELATIVE_TO_SELF, 0.0f,
                Animation.RELATIVE_TO_SELF, 1.0f);
