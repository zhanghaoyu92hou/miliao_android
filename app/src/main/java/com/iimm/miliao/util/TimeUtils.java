...
import static com.iimm.miliao.util.Constants.KEY_TIME_DIFFERENCE;
...
public class TimeUtils {
    ...
    public static final SimpleDateFormat time_format_1 = new SimpleDateFormat("yyyy-MM-dd HH:mm");
    ...
    public static String getFormatTime(long time) {
        return f_format.format(new Date(time));
    }
    /**
     * 在当前时间上加上多少毫秒，返回这个时间
     *
    ...
    public static String time_friendly_format2(long time) {
        return friendly_format2.format(new Date(time * 1000));
    }

    public static String time_s_long_2_str(long time) {
        return s_long_2_str(time * 1000);
    }

    public static String nearbyTimeString(long timestamp) {
        return sdfNearby.format(new Date(timestamp * 1000));
    }

    public static String time_ss_long_2_str(long time) {
        return ss_long_2_str(time * 1000);
    }

    public static long time_s_str_2_long(String dateString) {
    ...
    public static double time_current_time_double() {
        // 加上与服务器时间差值，
        long timeDifference = PreferenceUtils.getLong(MyApplication.getContext(), KEY_TIME_DIFFERENCE, 0L);
        return (System.currentTimeMillis() + timeDifference) / 1000D;
    }

    public static String time_long_to_hm_str(long time) {
        try {
    ...
    public static String time_long_to_chat_time_str(long time) {
        String date1 = time_s_long_2_str(time);
        String date2 = time_s_long_2_str(TimeUtils.time_current_time());
        if (date1.compareToIgnoreCase(date2) == 0) {// 是同一天
            return time_long_to_hm_str(time);
        } else {
    ...
    public static String long_to_yMdHm_str(long time) {
        return time_format_1.format(new Date(time));
    }

    public static long time_yMdHm_str_to_long(String time) {
        try {
            return time_format_1.parse(time).getTime() / 1000;
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return 0;
    ...
    public static int yMdHm_getYear(String dateString) {
        try {
            Date d = time_format_1.parse(dateString);
            return d.getYear() + 1900;// 年份是基于格林威治时间，所以加上1900
        } catch (ParseException e) {
            e.printStackTrace();
        }
    ...
    public static int yMdHm_getHours(String timeString) {
        try {
            Date date = time_format_1.parse(timeString);
            return date.getHours();
        } catch (ParseException e) {
            e.printStackTrace();
        }
    ...
    public static long getSpecialBeginTime(TextView textView, long time) {
        ...
        textView.setText(time_s_long_2_str(time));
        return time;
    }
    ...
    public static int time_age(long birthday) {
    ...
    public static boolean isSilenceInGroup(Friend friend) {
        if (friend == null) {
            return true;
        }
        return ((friend.getRoomTalkTime() > (TimeUtils.time_current_time())) || (friend.getRoomMyTalkTime() > (TimeUtils.time_current_time())));
    }
