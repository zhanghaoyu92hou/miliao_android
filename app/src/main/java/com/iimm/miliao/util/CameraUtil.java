...
public final class CameraUtil {
     ...
     * @return
     */
    public static Uri getOutputMediaContentUri(Context context, int type) {
        File file = getOutputMediaFile(context, type);
        if (file == null) {
            return null;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {//24
            ...
            String authority = new StringBuilder(packageName).append(Constants.FILE_PROVIDER_AUTHORITY).toString();
    ...
    public static Uri getOutputMediaContentUri(Context context, File file) {
        if (file == null) {
            return null;
        }
        /* longhui20171107 兼容Android7运行Uri.fromFile修改，SDK大于24就出错*/
     ...
     *
     * @param activity
     * @param originalContentOrFileUri 原始图片位置   和 getOutputMediaContentUri 中的Uri保持一致，如果是SDK >= 24 则为ContentUri，否则为FileUri
     * @param cropFileUri              裁减后图片的输出位置，两个地址最好不一样。如果一样的话，有的手机上面无法保存裁减的结果
     * @return
     */
    public static void cropImageAfterTakePicture(Activity activity, Uri originalContentOrFileUri, Uri cropFileUri, int requestCode, int aspectX, int aspectY, int outputX, int outputY) {
        if (originalContentOrFileUri == null) {
            return;
        }
        ...
        intent.setDataAndType(originalContentOrFileUri, "image/*");
    ...
    public static void cropImage(Activity activity, Uri originalFileUri, Uri outputFileUri, int requestCode, int aspectX, int aspectY, int outputX,
        ...
        final Intent intent = new Intent("com.android.camera.action.CROP");
        String filePath = FileUtil.getFilePath(activity, originalFileUri);
        File file = new File(filePath);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            originalFileUri = FileProvider.getUriForFile(activity, activity.getPackageName() + Constants.FILE_PROVIDER_AUTHORITY, file);
            intent.setDataAndType(originalFileUri, "image/*");
        } else {
            originalFileUri = Uri.fromFile(new File(filePath));
            intent.setDataAndType(originalFileUri, "image/*");
        }
        intent.putExtra("crop", "true");
        intent.putExtra("aspectX", aspectX);
