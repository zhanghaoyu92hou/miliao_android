...
public abstract class MapHelper {
    ...
    public static MapHelper getInstance() {
        MapHelper mapHelper = getInstance(sMapType);
        if (mapHelper == null) {
            mapHelper = BaiduMapHelper.getInstance(context);
        }
        return mapHelper;
    }
    ...
    public static MapHelper getInstance(MapType mapType) {
        String[] permission = new String[]{Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.READ_PHONE_STATE};
        if (!PermissionUtil.checkSelfPermissions(context, permission)) {
            //权限没有完全开启
            final MapHelper[] result = {null};
            AndPermissionUtils.position(context, new OnPermissionClickListener() {
                @Override
                public void onSuccess() {
                    switch (mapType) {
                        case BAIDU:
                            result[0] = BaiduMapHelper.getInstance(context);
                            break;
                        case GOOGLE:
                            result[0] = GoogleMapHelper.getInstance(context);
                            break;
                        default:
                            break;
                    }
                    if (!result[0].isAvailability()) {
                        Log.d(TAG, "getInstance: 设备不支持该地图, " + mapType);
                        result[0] = getInstance(DEFAULT_MAP_TYPE);
                    }
                }
            ...
            boolean isAlwaysDenied = AndPermission.hasAlwaysDeniedPermission(context, permission);
            if (result[0] == null || isAlwaysDenied) {
                //权限被永久拒绝
                ToastUtil.showToast(context, R.string.please_open_some_permissions);
            }
            return result[0];
        } else {
            ...
            if (result == null || !result.isAvailability()) {
                Log.d(TAG, "getInstance: 设备不支持该地图, " + mapType);
     ...
     * 判断是否在国内
     *
     * @param booleanOnSuccessListener
     * @param onErrorListener
     */
    public abstract void isInChina(OnSuccessListener<Boolean> booleanOnSuccessListener, OnErrorListener onErrorListener);
