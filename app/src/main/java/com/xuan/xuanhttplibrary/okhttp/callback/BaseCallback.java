...
public abstract class BaseCallback<T> implements Callback {
    ...
    public void onResponse(Call call, Response response) throws IOException {
        Log.i(HttpUtils.TAG, "服务器数据包：" + response.code());
    ...
    protected void successData(final ObjectResult<T> data) {
        if (data != null && data.getCurrentTime() > 0) {
            TimeUtils.responseTime(data.getCurrentTime());
            //Log.i(TAG, "successData ServerTime: " + TimeUtils.long_to_yMdHm_str(data.getCurrentTime()));
        }
