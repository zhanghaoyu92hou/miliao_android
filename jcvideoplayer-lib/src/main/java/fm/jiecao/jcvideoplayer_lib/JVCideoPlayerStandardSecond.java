...
public class JVCideoPlayerStandardSecond extends JCVideoPlayer {
    ...
    private void downloadUrl(final String mVideoPath) {
        final String path = getFileDir() + File.separator +
                getLastName(mVideoPath);
        if (mVideoPath.startsWith("http")) {
            ...
            final File copyFile = new File(path);
            if (!copyFile.exists()) {
                new Thread() {
                    @Override
                    public void run() {
                        super.run();
                        copyFile(mVideoPath.substring(7, mVideoPath.length()), path);
                        ContentResolver localResolver = mContext.getContentResolver();
                        ContentValues localContentValue = getVideoContentValues(copyFile, System.currentTimeMillis());
